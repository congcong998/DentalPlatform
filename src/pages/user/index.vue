...
<template>
  <view class="min-h-screen bg-gray-100">
    <!-- 顶部背景 -->
    <view class="from-blue-500 to-blue-600 bg-gradient-to-br pb-20">
      <!-- 状态栏占位 -->
      ...
      <!-- 用户信息 -->
      <view class="mt-4 px-5" @click="handleLogin">
    ...
    <view v-else class="mx-4 mt-[-60px] overflow-hidden rounded-xl bg-white p-6 shadow-md">
      <view class="flex flex-col items-center">
        ...
        <button
          class="mt-3 rounded-full bg-blue-500 px-8 py-2 text-sm text-white"
          @click="handleLogin"
        >
          立即登录
        </button>
    ...
    <!-- 退出登录 -->
    <view v-if="isLogin" class="mx-4 mb-8 mt-6">
      <button
        class="w-full border border-gray-200 rounded-xl bg-white py-3 text-red-500 shadow-sm active:bg-gray-50"
        @click="handleLogout"
      >
        退出登录
      </button>
